...
function sortAscending(hotel1, hotel2, value) {
  return hotel1[value] - hotel2[value];
}

function sortDescending(hotel1, hotel2, value) {
  return hotel2[value] - hotel1[value];
}
...
app.get('/hotels/sort/pricing', (req, res) => {
  ...
  console.log('Sorting on Pricing');
  if (pricing === 'low-to-high') {
    let hotelsCopy = hotels.slice();
    hotelsCopy.sort((hotel1, hotel2) => sortAscending(hotel1, hotel2, 'price'));
...
app.get('/hotels/sort/rating', (req, res) => {
  ...
  if (rating === 'low-to-high') {
    hotelsCopy.sort((hotel1, hotel2) =>
      sortAscending(hotel1, hotel2, 'rating')
    );
    res.json({ hotels: hotelsCopy });
  } else {
    hotelsCopy.sort((hotel1, hotel2) =>
      sortDescending(hotel1, hotel2, 'rating')
    );
    res.json({ hotels: hotelsCopy });
  }
});
...
app.get('/hotels/sort/reviews', (req, res) => {
  ...
  if (reviews === 'least-to-most') {
    hotelsCopy.sort((hotel1, hotel2) =>
      sortAscending(hotel1, hotel2, 'reviews')
    );
    res.json({ hotels: hotelsCopy });
  } else {
    hotelsCopy.sort((hotel1, hotel2) =>
      sortDescending(hotel1, hotel2, 'reviews')
    );
    res.json({ hotels: hotelsCopy });
  }
});

//Sorting based on amenity
app.get('/hotels/filter/amenity', (req, res) => {
  let amenity = req.query.amenity.toLowerCase();
  console.log('amenity ' + amenity);
  let hotelsCopy = hotels.slice();
  let results = hotelsCopy.filter(
